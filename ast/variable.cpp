...
GlobalVariable::GlobalVariable(Type *type, const std::string &id,
                               int pointer_count)
    : type_(type), id_(id), pointer_count_(pointer_count), marshal_info_(NULL),
      container_(NULL), accessor_(NULL) {}
...
Type *GlobalVariable::type() { return this->type_; }

const std::string &GlobalVariable::identifier() const { return this->id_; }

void GlobalVariable::set_identifier(const std::string &id) { this->id_ = id; }
...
void GlobalVariable::resolve_types(LexicalScope *ls) {
  ...
  if (this->type_->num() == INITIALIZE_TYPE) {
    ...
    if (t == 0x0) {
      std::cout << "Error: could not resolve type " << this->type_->name()
                << std::endl;
      return;
    }
...
void GlobalVariable::initialize_type() {
  if (this->type_->num() == INITIALIZE_TYPE) {
    ...
    it->initialize();
    this->type_ = it->type_;
  } else if (this->type_->num() == PROJECTION_TYPE ||
             this->type_->num() == PROJECTION_CONSTRUCTOR_TYPE) {
    ProjectionType *pt = dynamic_cast<ProjectionType *>(this->type_);
    Assert(pt != 0x0, "Error: dynamic cast to Projection type failed\n");
...
void GlobalVariable::create_container_variable(LexicalScope *ls) {
  ...
  if (this->type_->num() == INITIALIZE_TYPE) { // initialize type
    ...
    if (tmp->num() != PROJECTION_TYPE &&
        tmp->num() != PROJECTION_CONSTRUCTOR_TYPE) {
      return;
    }
  }
  ...
  // really need to make variable non abstract and get rid of unnecessary
  // variables.

  const std::string &name = container_name(
      append_strings(std::string("_"), construct_list_vars(this)));
...
Variable *GlobalVariable::container() { return this->container_; }
...
/* parameter */
Parameter::Parameter()
    : in_(false), out_(false), alloc_callee_(false), alloc_caller_(false),
      dealloc_callee_(false), dealloc_caller_(false), bind_callee_(false),
      bind_caller_(false), type_(NULL), name_(NULL), marshal_info_(NULL),
      accessor_(NULL), pointer_count_(0), container_(NULL) {}

Parameter::Parameter(Type *type, const std::string &name, int pointer_count)
    : in_(false), out_(false), alloc_callee_(false), alloc_caller_(false),
      dealloc_callee_(false), dealloc_caller_(false), bind_callee_(false),
      bind_caller_(false), type_(type), name_(name), marshal_info_(NULL),
      accessor_(NULL), pointer_count_(pointer_count), container_(NULL) {}

Parameter::Parameter(const Parameter &other)
    : marshal_info_(NULL), accessor_(NULL) {
  this->type_ = other.type_->clone();
...
void Parameter::create_container_variable(LexicalScope *ls) {
  ...
  if (this->type_->num() == PROJECTION_TYPE) {
    ...
    Assert(container != 0x0,
           "Error: could not dynamically cast to projection\n");

    // really need to make variable non abstract and get rid of unnecessary
    // variables.

    const std::string &name =
        container_name(append_strings("_", construct_list_vars(this)));
...
Variable *Parameter::container() { return this->container_; }
...
Type *Parameter::type() { return this->type_; }

int Parameter::pointer_count() { return this->pointer_count_; }

void Parameter::set_pointer_count(int pcount) { this->pointer_count_ = pcount; }
...
Variable *Parameter::accessor() { return this->accessor_; }
...
void Parameter::set_in(bool b) { this->in_ = b; }

void Parameter::set_out(bool b) { this->out_ = b; }

void Parameter::set_alloc_caller(bool b) { this->alloc_caller_ = b; }

void Parameter::set_alloc_callee(bool b) { this->alloc_callee_ = b; }

void Parameter::set_dealloc_caller(bool b) { this->dealloc_caller_ = b; }

void Parameter::set_dealloc_callee(bool b) { this->dealloc_callee_ = b; }

bool Parameter::in() { return this->in_; }

bool Parameter::out() { return this->out_; }

bool Parameter::alloc_caller() { return this->alloc_caller_; }
bool Parameter::alloc_callee() { return this->alloc_callee_; }

bool Parameter::dealloc_caller() { return this->dealloc_caller_; }
...
ReturnVariable::ReturnVariable(Type *return_type, int pointer_count,
                               const std::string &id)
    : name_(id), type_(return_type), marshal_info_(NULL), accessor_(NULL),
      pointer_count_(pointer_count), container_(NULL), in_(false), out_(false),
      alloc_caller_(false), alloc_callee_(false) {}
...
void ReturnVariable::set_identifier(const std::string &id) { this->name_ = id; }

Type *ReturnVariable::type() { return this->type_; }
...
int ReturnVariable::pointer_count() { return this->pointer_count_; }
...
Variable *ReturnVariable::accessor() { return this->accessor_; }
...
void ReturnVariable::set_alloc_caller(bool b) { alloc_caller_ = b; }

void ReturnVariable::set_alloc_callee(bool b) { alloc_callee_ = b; }

void ReturnVariable::set_dealloc_caller(bool b) {
  std::cout << "ReturnVariable::set_dealloc_caller: error this operation not "
               "allowed\n";
}

void ReturnVariable::set_dealloc_callee(bool b) {
  std::cout << "ReturnVariable::set_dealloc_callee: error this operation not "
               "allowed\n";
}

bool ReturnVariable::in() { return this->in_; }

bool ReturnVariable::out() { return this->out_; }

bool ReturnVariable::alloc_caller() { return this->alloc_caller_; }

bool ReturnVariable::alloc_callee() { return this->alloc_callee_; }

bool ReturnVariable::dealloc_caller() {
  std::cout
      << "ReturnVariable::dealloc_caller: error this operation not allowed\n";
  return false;
}

bool ReturnVariable::dealloc_callee() {
  std::cout
      << "ReturnVariable::dealloc_callee: error this operation not allowed\n";
  return false;
}
...
ProjectionField::ProjectionField(Type *field_type,
                                 const std::string &field_name,
                                 int pointer_count)
    : in_(false), out_(false), alloc_callee_(false), alloc_caller_(false),
      dealloc_callee_(false), dealloc_caller_(false), bind_callee_(false),
      bind_caller_(false), type_(field_type), field_name_(field_name),
      accessor_(NULL), pointer_count_(pointer_count), marshal_info_(NULL),
      container_(NULL) {}
...
void ProjectionField::create_container_variable(LexicalScope *ls) {
  ...
  int err;
  Type *container_t = ls->lookup(container_name(tmp->name()), &err);
  std::cout << "looking up container " << container_name(tmp->name())
            << std::endl;
  Assert(container_t != 0x0, "Error: could not find container in scope\n");
...
int ProjectionField::pointer_count() { return this->pointer_count_; }
...
Variable *ProjectionField::accessor() { return this->accessor_; }
...
void ProjectionField::set_in(bool b) { this->in_ = b; }

void ProjectionField::set_out(bool b) { this->out_ = b; }
...
void ProjectionField::set_dealloc_callee(bool b) { this->dealloc_callee_ = b; }

bool ProjectionField::in() { return this->in_; }

bool ProjectionField::out() { return this->out_; }

bool ProjectionField::alloc_callee() { return this->alloc_callee_; }

bool ProjectionField::alloc_caller() { return this->alloc_caller_; }

bool ProjectionField::dealloc_caller() { return this->dealloc_caller_; }
...
Variable *FPParameter::container() { return this->container_; }
...
Type *FPParameter::type() { return this->type_; }

int FPParameter::pointer_count() { return this->pointer_count_; }
...
void FPParameter::set_identifier(const std::string &id) { return; }
