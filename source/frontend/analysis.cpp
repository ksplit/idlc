...
namespace idlc {
	namespace {
		...
		public:
			annotation_walk(annotation_bitfield default_with) : m_default_with {default_with}
			{
		...
		passed_type
		instantiate_projection(proj_def& node, node_ptr<projection>& cached, annotation_bitfield default_with)
		{
			if (cached) {
		...
		class rpc_pointer_sanity_walk : public ast_walk<rpc_pointer_sanity_walk> {
		public:
			bool visit_type_rpc(const type_rpc& node)
			{
				...
				if (node.definition->kind != rpc_def_kind::indirect) {
					std::cout << "[error] an RPC must be declared as an rpc_ptr to be used as a field type\n";
					return false;
				}
...
std::optional<idlc::rpc_vec> idlc::generate_rpc_pgraphs(file& root)
{
	...
	rpc_pointer_sanity_walk static_rpc_walk {};
