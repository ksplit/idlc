...
namespace idlc {
	...
	enum class annotation_kind {
		...
		alloc_stack_caller = 1 << 12,
		alloc_stack_callee = 1 << 13,
		shared			= 1 << 14,
		unused			= 1 << 15,
		...
		is_ptr			= is_bind | is_dealloc | is_alloc | is_ioremap | is_alloc_once | is_alloc_stack | shared,
	...
	struct annotation {
		...
		annotation(annotation_kind kind, ident share_global, ident verbatim) :
			kind {kind},
