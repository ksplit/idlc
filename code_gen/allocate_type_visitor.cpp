...
CCSTStatement *AllocateTypeVisitor::visit(FloatType *ft, Variable *v) {
  ...
  void_star_quals.push_back(
      new CCSTSimpleTypeSpecifier(CCSTSimpleTypeSpecifier::VoidTypeSpec));

  CCSTTypeName *void_star =
      new CCSTTypeName(void_star_quals, new CCSTPointer());
  ...
  while (p_count > 1) {
    ...
    v->set_pointer_count(p_count_save - p_count +
                         1); // 0 then 1. when its 1 it isnt derefing
    statements.push_back(new CCSTExprStatement(new CCSTAssignExpr(
        access(v), equals(), function_call("kzalloc", kzalloc_args))));
  ...
  v->set_pointer_count(p_count_save - p_count +
                       1); // p count will either be 1 or will be p coutn save
  std::vector<CCSTAssignExpr *> kzalloc_args;
...
CCSTStatement *AllocateTypeVisitor::visit(VoidType *vt, Variable *v) {
  ...
  decs_order.push_back(new CCSTInitDeclarator(
  ...
  decs_gva.push_back(new CCSTInitDeclarator(
      new CCSTDeclarator(NULL, new CCSTDirectDecId(v->identifier() + "_gva"))));
...
CCSTStatement *AllocateTypeVisitor::visit(ProjectionType *pt, Variable *v) {
  ...
  while (p_count > 1) {
    ...
    std::string *goto_alloc =
        new std::string("fail_alloc" + std::to_string(p_count));
    statements.push_back(if_cond_fail_goto(
        new CCSTUnaryExprCastExpr(Not(), access(v)), "kzalloc", *goto_alloc));
...
CCSTStatement *AllocateTypeVisitor::visit(ProjectionConstructorType *pt,
                                          Variable *v) {
  std::vector<CCSTDeclaration *> declarations;
  std::vector<CCSTStatement *> statements;
