...
CCSTFuncDef *function_definition(CCSTDeclaration *function_declaration,
  ...
  Assert(function_declaration->decs_.size() == 1,
         "Error: More than one initializer/declarator in function declaration");

  CCSTDeclarator *func =
      dynamic_cast<CCSTDeclarator *>(function_declaration->decs_.at(0));
  Assert(func != 0, "Error: dynamic cast from CCSTInitDeclarator to "
                    "CCSTDeclarator has failed!");

  std::vector<CCSTDeclaration *>
      decs; // not sure what this is for. unnecessary?

  return new CCSTFuncDef(function_declaration->specifier_, func, decs, body,
                         function_declaration->attributes_);
}
  ...
  for (auto p : params) {
    ...
    if (p->type()->num() == FUNCTION_TYPE) {
      ...
      std::vector<CCSTDecSpecifier *> new_fp_return_type =
          type2(f->return_var_->type());

      /// loop through rpc parameters and add them to the parameters for the new
      /// fp
...
function_pointer_function_declaration(Rpc *r, const std::string &_postfix) {
  ...
  all_params.insert(all_params.end(), r_hidden_args.begin(),
                    r_hidden_args.end());
...
CCSTPostFixExpr *access(Variable *v) {
  if (v->accessor() == 0x0) {
    ...
    if (v->accessor()->pointer_count() == 0) {
      ...
      if (pc > 1) {
        int tmp = pc - 1;
        CCSTCastExpr *init = new CCSTPostFixExprAccess(
            access(v->accessor()), object_access_t, v->identifier());
        while (tmp > 0) {
          init = dereference(init);
          tmp -= 1;
        }
        ...
        return new CCSTPostFixExprAccess(access(v->accessor()), object_access_t,
                                         v->identifier());
      }
    } else {
      int pc = v->pointer_count();
      if (pc > 1) {
        int tmp = pc - 1;
        CCSTCastExpr *init = new CCSTPostFixExprAccess(
            access(v->accessor()), pointer_access_t, v->identifier());
        while (tmp > 0) {
          init = dereference(init);
          tmp -= 1;
        }
        ...
        return new CCSTPostFixExprAccess(access(v->accessor()),
                                         pointer_access_t, v->identifier());
      }
    }
  }
}
