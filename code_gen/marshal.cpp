...
CCSTStatement *marshal_void_pointer(Variable *v) {
  ...
  decs_sret.push_back(new CCSTInitDeclarator(
      new CCSTDeclarator(NULL, new CCSTDirectDecId("sync_ret"))));
  decs_off.push_back(new CCSTInitDeclarator(new CCSTDeclarator(
      NULL, new CCSTDirectDecId(v->identifier() + "_offset"))));
  decs_cptr.push_back(new CCSTInitDeclarator(new CCSTDeclarator(
      NULL, new CCSTDirectDecId(v->identifier() + "_cptr"))));
  decs_sz.push_back(new CCSTInitDeclarator(new CCSTDeclarator(
      NULL, new CCSTDirectDecId(v->identifier() + "_mem_sz"))));
  ...
  gva_args.push_back(new CCSTCastExpr(new CCSTTypeName(spec_ull2, NULL),
                                      new CCSTPrimaryExprId(v->identifier())));
  virt_cptr_args.push_back(function_call("__gva", gva_args));

  virt_cptr_args.push_back(new CCSTUnaryExprCastExpr(
      reference(), new CCSTPrimaryExprId(v->identifier() + "_cptr")));

  virt_cptr_args.push_back(new CCSTUnaryExprCastExpr(
      reference(), new CCSTPrimaryExprId(v->identifier() + "_mem_sz")));

  virt_cptr_args.push_back(new CCSTUnaryExprCastExpr(
      reference(), new CCSTPrimaryExprId(v->identifier() + "_offset")));
...
CCSTStatement *marshal_void_delayed(Variable *v) {
  ...
  ilog2_args.push_back(new CCSTShiftExpr(
      rightshift_t, new CCSTPrimaryExprId(v->identifier() + "_mem_sz"),
  ...
  statements.push_back(new CCSTExprStatement(new CCSTPostFixExprAssnExpr(
      new CCSTPrimaryExprId("lcd_set_r0"), set_r0_args)));

  statements.push_back(new CCSTExprStatement(new CCSTPostFixExprAssnExpr(
      new CCSTPrimaryExprId("lcd_set_r1"), set_r1_args)));
...
CCSTStatement *marshal_variable(Variable *v, const std::string &direction,
  ...
  if (v->container() != 0x0) {
    // marshal container
    std::cout << "trying to marshal container " << v->container()->identifier()
              << std::endl;
  ...
  if (v->type()->num() == PROJECTION_TYPE ||
      v->type()->num() == PROJECTION_CONSTRUCTOR_TYPE) { // projection
    // loop through fields
...
marshal_variable_callee(Variable *v, Channel::ChannelType type) {
  std::vector<CCSTStatement *> statements;

  if (v->container() != 0x0) {
    if (v->container()->out()) {
      std::vector<CCSTStatement *> tmp_stmts =
          marshal_variable_callee(v->container(), type);
    ...
    for (auto pf : *pt) {
      if (pf->out()) {
        std::vector<CCSTStatement *> tmp_stmt =
            marshal_variable_callee(pf, type);
...
std::vector<CCSTStatement *>
marshal_variable_no_check(Variable *v, Channel::ChannelType type) {
  std::vector<CCSTStatement *> statements;
