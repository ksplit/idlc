...
std::vector<CCSTSpecifierQual *> type(Type *t) {
  ...
  case INTEGER_TYPE: // int type case
  {
    ...
    case pt_int_t: {
      specifier.push_back(new CCSTSimpleTypeSpecifier(
  ...
  default: {
    std::cout << "Received " << type_number_to_name(num) << " with name "
              << t->name() << std::endl;
...
CCSTTypeName *type_cast(Type *t, int pointer_count) {
  ...
  CCSTAbstDeclarator *pointers =
      new CCSTAbstDeclarator(pointer(pointer_count), 0x0);
  ...
  default: { Assert(1 == 0, "Error: Should never get here\n"); }
  }
    ...
    if (pf->type()->num() == FUNCTION_TYPE) {
      ...
      specifier.push_back(new CCSTSimpleTypeSpecifier(
          CCSTSimpleTypeSpecifier::
              IntegerTypeSpec)); // type2(f->return_var_->type());
      ...
      struct_decs.push_back(new CCSTStructDeclarator(new CCSTDeclarator(
      ...
      field_decs.push_back(new CCSTStructDeclaration(
          specifier, new CCSTStructDecList(struct_decs)));
    } else {
      field_decs.push_back(new CCSTStructDeclaration(
          type(pf->type()), new CCSTStructDeclarator(new CCSTDeclarator(
...
CCSTDeclaration *struct_pointer_declaration(const std::string &struct_name,
  ...
  std::vector<CCSTInitDeclarator *> decs;
  decs.push_back(
      new CCSTDeclarator(new CCSTPointer(), new CCSTDirectDecId(var_name)));
...
CCSTStatement *kzalloc_structure(const std::string &struct_name,
  ...
  kzalloc_args.push_back(new CCSTUnaryExprSizeOf(new CCSTUnaryExprCastExpr(
      new CCSTUnaryOp(unary_mult_t), new CCSTPrimaryExprId(struct_name))));
  kzalloc_args.push_back(new CCSTEnumConst("GFP_KERNEL"));
  return new CCSTExprStatement(
      new CCSTAssignExpr(new CCSTPrimaryExprId(var_name), equals(),
