 ...
 * Assume structure a variable belongs to has been previously allocated, at the
 * obvious name. This function should never be called on a variable whose type
 * is a projection.
 */
...
unmarshal_return_variable_no_check(Variable *v, Channel::ChannelType type) {
  ...
  statements.push_back(new CCSTExprStatement(new CCSTAssignExpr(
      access(v), equals(), function_call(func_name, access_reg_args))));

  return statements;
}

std::vector<CCSTStatement *>
unmarshal_variable_no_check(Variable *v, Channel::ChannelType type) {
  std::vector<CCSTStatement *> statements;
  ...
  if (v->type()->num() == PROJECTION_TYPE ||
    ...
    for (auto pf : *pt) {
      std::vector<CCSTStatement *> tmp_stmts =
          unmarshal_variable_no_check(pf, type);
...
unmarshal_variable_caller(Variable *v, Channel::ChannelType type) {
  ...
  if (v->type()->num() == PROJECTION_TYPE ||
    ...
    for (auto pf : *pt) {
      if (pf->out()) {
        std::vector<CCSTStatement *> tmp_stmt =
            unmarshal_variable_caller(pf, type);
        statements.insert(statements.end(), tmp_stmt.begin(), tmp_stmt.end());
      }
    }

  } else {
    Assert(v->marshal_info() != 0x0,
           "Error: variable has no marshalling information\n");
...
std::vector<CCSTStatement *> unmarshal_void_pointer(Variable *v) {
  ...
  cptr_alloc_args.push_back(new CCSTUnaryExprCastExpr(
      reference(), new CCSTPrimaryExprId(v->identifier() + "_cptr")));
  ...
  map_virt_args.push_back(new CCSTUnaryExprCastExpr(
      reference(), new CCSTPrimaryExprId(v->identifier() + "_gva")));
  ...
  statements.push_back(new CCSTExprStatement(new CCSTPostFixExprAssnExpr(
      new CCSTPrimaryExprId("lcd_set_cr0"), set_cr0_args)));
  ...
  statements.push_back(new CCSTExprStatement(new CCSTPostFixExprAssnExpr(
      new CCSTPrimaryExprId("lcd_set_cr0"), set_cr0_args2)));
  ...
  statements.push_back(if_cond_fail(new CCSTPrimaryExprId("sync_ret"),
...
unmarshal_variable_callee(Variable *v, Channel::ChannelType type) {
  ...
  if (v->type()->num() == PROJECTION_TYPE ||
    ...
    for (auto pf : *pt) {
      if (pf->in()) {
        std::vector<CCSTStatement *> tmp_stmt =
            unmarshal_variable_callee(pf, type);
        statements.insert(statements.end(), tmp_stmt.begin(), tmp_stmt.end());
      }
    }
    ...
    if (v->container() != 0x0) {
      ...
      statements.push_back(new CCSTExprStatement(
          new CCSTAssignExpr(access(v_container_real_field), equals(),
                             function_call(reg_func, reg_func_args_empty))));
    } else {
      // no container
      statements.push_back(new CCSTExprStatement(new CCSTAssignExpr(
          access(v), equals(), function_call(reg_func, reg_func_args_empty))));
    }
  }
  return statements;
}
...
unmarshal_container_refs_caller(Variable *v, Channel::ChannelType type) {
  ...
  if (v->type()->num() == PROJECTION_TYPE ||
    ...
    for (auto pf : *pt) {
      if (pf->out()) {
        std::vector<CCSTStatement *> tmp_stmts =
            unmarshal_container_refs_caller(pf, type);
        statements.insert(statements.end(), tmp_stmts.begin(), tmp_stmts.end());
      }
    }
  }
